...
| 模块 | 功能 | 状态 |
|---|---|---|
| 屏蔽体系 | 国内分类名单（短视频/视频娱乐/社交/资讯/购物/沟通工具 6 类）+ 自定义域名 + 每日限额（分钟/天）+ 临时放行 5/15/30 分钟（菜单选择）+ 一键清理残留 + **屏蔽绑定专注（保存配置→专注开始时自动生效→结束时自动解除）** | ✅ 完整可用 |
...
| 统计 | 今日定心/专注分钟/连续天数 + 近 7 天柱状 + 本月热力 + 今日使用分布 + 分享卡片（保存PNG+复制剪贴板）+ 空状态引导 | ✅ v0.4.3 |
| 限额提醒 | 每日限额超限 → 托盘气泡（按域名每日去重） | ✅ |
| 系统集成 | 托盘常驻（关窗/最小化均到托盘）+ 一键屏蔽抖音/B站快捷项 + 前台窗口采集（仅进程名+标题）+ SQLite 本地存储 + 全局 InfoBar 通知条 | ✅ v0.4.3 |
...
**测试**
- 单元测试 48/48 通过
- 构建 0 错误，冒烟启动通过（托盘/导航/活动追踪/付费逻辑日志正常）
