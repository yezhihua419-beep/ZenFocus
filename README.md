...
| 模块 | 功能 | 状态 |
|---|---|---|
| 屏蔽体系 | 国内分类名单（短视频/视频娱乐/社交/资讯/购物/沟通工具 6 类）+ 自定义域名 + 每日限额（分钟/天）+ 临时放行 5/15/30 分钟（菜单选择）+ 一键清理残留 + **屏蔽绑定专注（保存配置→专注开始时自动生效→结束时自动解除）** + **手动屏蔽（不依赖专注计时，随时开启/关闭）** | ✅ 完整可用 |
...
| 统计 | 今日定心/专注分钟/连续天数 + 近 7 天柱状 + 本月热力 + 今日使用分布 + 分享卡片（保存PNG+复制剪贴板+质量分）+ 空状态引导 + **专注质量评分（时长基础分-分心扣分）** + **分心模式识别（近7天最易分心时段）** + **付费功能预览钩子** | ✅ v0.4.3 + v0.5.0 + v0.5.1 |
| 限额提醒 | 每日限额超限 → 托盘气泡（按域名每日去重） | ✅ |
| 系统集成 | 托盘常驻（关窗/最小化均到托盘）+ 一键屏蔽抖音/B站快捷项 + 前台窗口采集（仅进程名+标题）+ SQLite 本地存储 + 全局 InfoBar 通知条 + **全局快捷键（Ctrl+Alt+F 开始/暂停专注）** + **编码检测场景推荐（持续编码2分钟→推荐工作场景）** | ✅ v0.4.3 + v0.5.0 + v0.5.1 |
| 商业位 | 免费版自定义域名限3个 + 场景自定义为付费功能（免费1个额度）+ 手机伴侣为付费功能 + 激活占位（待接支付） | ✅ 预留 |
| **移动联动** | **局域网 Web 伴侣页（手机浏览器扫码访问，查看专注统计/远程开始结束专注/专注状态同步/同伴专注状态，零云端零配置）** | ✅ v0.4.0 + v0.5.1 |
